...
const char * const BITCOIN_CONF_FILENAME = "bitglo.conf";
const char * const BITCOIN_PID_FILENAME = "bitglod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bitglo" is a composite category enabling all Bitglo-related debug output
            if(ptrCategory->count(string("bitglo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitgloCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitgloCore
    // Mac: ~/Library/Application Support/BitgloCore
    // Unix: ~/.bitglocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitgloCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitgloCore";
#else
    // Unix
    return pathRet / ".bitglocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitglo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bitglo.conf
